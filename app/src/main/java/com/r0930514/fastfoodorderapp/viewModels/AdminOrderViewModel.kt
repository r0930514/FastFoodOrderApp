...
class AdminOrderViewModel(
    ...
    private fun fetchOrderList(){
        viewModelScope.launch {
            try {
                ordersRepository.getAllOrders(token.value).collect { list ->
    ...
    fun doneOrder(id: String){
        viewModelScope.launch {
            try {
                ordersRepository.doneOrder(token.value, id).collect {
                    fetchOrderList()
                }
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    companion object{
        val Factory: ViewModelProvider.Factory = viewModelFactory {
            initializer {
                ...
                AdminOrderViewModel(ordersRepository, UserStateRepository(appContext.dataStore))
            }
        }
    }
}